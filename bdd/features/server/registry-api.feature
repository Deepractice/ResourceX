...
Feature: Registry HTTP Server
  ...
  Base URL: https://registry.deepractice.dev/api/v1
  ...
  Scenario: Get resource manifest
    ...
    When I GET "/api/v1/resource?locator=hello.text@1.0.0"
    Then the response status should be 200
  ...
  Scenario: Get resource with domain and path
    ...
    When I GET "/api/v1/resource?locator=deepractice.dev/prompts/assistant.prompt@2.0.0"
    Then the response status should be 200
    And the response should be JSON with:
      | field   | value           |
  ...
  Scenario: Get non-existent resource returns 404
    When I GET "/api/v1/resource?locator=not-exist.text@1.0.0"
  ...
  Scenario: HEAD returns 200 for existing resource
    ...
    When I HEAD "/api/v1/resource?locator=hello.text@1.0.0"
    Then the response status should be 200

  @exists
  Scenario: HEAD returns 404 for non-existent resource
    When I HEAD "/api/v1/resource?locator=not-exist.text@1.0.0"
    Then the response status should be 404
  ...
  Scenario: Get resource archive
    Given a published resource "hello.text@1.0.0" with content "Hello World"
    When I GET "/api/v1/content?locator=hello.text@1.0.0"
    Then the response status should be 200
  ...
  Scenario: Get content for non-existent resource returns 404
    When I GET "/api/v1/content?locator=not-exist.text@1.0.0"
    Then the response status should be 404
  ...
  Scenario: Search with query filter
    Given published resources:
      ...
      | world.text@1.0.0     |
      | greeting.json@2.0.0  |
    When I GET "/api/v1/search?q=text"
    Then the response status should be 200
  ...
  Scenario: Search with pagination
    ...
    When I GET "/api/v1/search?limit=5&offset=10"
    Then the response status should be 200
  ...
  Scenario: Publish resource with multipart form
    ...
    When I POST "/api/v1/publish" with multipart:
      | field    | value         |
      | manifest | <manifest>    |
      | archive  | <archive>     |
  ...
  Scenario: Publish overwrites existing version
    ...
    And I GET "/api/v1/content?locator=overwrite.text@1.0.0"
  ...
  Scenario: Delete existing resource
    ...
    When I DELETE "/api/v1/resource?locator=todelete.text@1.0.0"
    Then the response status should be 204
    And the resource "todelete.text@1.0.0" should not exist

  @delete @error
  Scenario: Delete non-existent resource returns 404
    When I DELETE "/api/v1/resource?locator=not-exist.text@1.0.0"
    Then the response status should be 404
  ...
  Scenario: OPTIONS returns CORS headers
    When I OPTIONS "/api/v1/resource"
    Then the response status should be 204
