...
Feature: Registry Add and Publish
  ...
  Scenario: Add resource from RXR object
    Given a resource "hello.text@1.0.0" with content "Hello World"
    When I add the resource to registry
    Then the resource should exist in local registry
    And I should be able to resolve "hello.text@1.0.0"
    And the resolved content should be "Hello World"
  ...
  Scenario: Add resource from directory path
    Given a resource directory "./test-resource" with:
      ...
      | content.txt   | Hello from directory |
    When I add "./test-resource" to registry
    Then the resource should exist in local registry
    And I should be able to resolve "greeting.text@1.0.0"
    And the resolved content should be "Hello from directory"

  @add
  Scenario: Add resource twice overwrites
    Given a resource "test.text@1.0.0" with content "Version 1"
    When I add the resource to registry
    And I create a resource "test.text@1.0.0" with content "Version 2"
    And I add the resource to registry
    Then the resolved content should be "Version 2"
  ...
  Scenario: Link development directory
    Given a resource directory "./dev-resource" with:
      ...
      | content.txt   | Original content |
    When I link "./dev-resource" to registry
    Then the resource should exist in local registry
    And I should be able to resolve "dev.text@1.0.0"
    And the resolved content should be "Original content"
  ...
  Scenario: Link reflects file changes immediately
    Given a resource directory "./live-resource" with:
      ...
      | content.txt   | Before change |
    When I link "./live-resource" to registry
    And I modify "./live-resource/content.txt" to "After change"
    And I resolve "live.text@1.0.0"
    Then the resolved content should be "After change"
  ...
  Scenario: Add does not reflect file changes
    ...
    And I modify "./static-resource/content.txt" to "After change"
    And I resolve "static.text@1.0.0"
    Then the resolved content should be "Before change"
