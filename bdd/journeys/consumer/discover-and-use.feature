...
Feature: Discover and use resources
  ...
  Scenario: Search and use a resource
    ...
    Then the command should succeed
    When I run consumer command "rx ingest localhost:3098/helper:1.0.0"
    Then the command should succeed
  ...
  Scenario: Pull resource for offline use
    ...
    When I run consumer command "rx ingest localhost:3098/offline:1.0.0"
    Then the command should succeed
  ...
  Scenario: Use specific version when multiple exist
    ...
    When I run consumer command "rx ingest localhost:3098/versioned:1.0.0"
    Then the output should contain "Version 1"

    When I run consumer command "rx ingest localhost:3098/versioned:3.0.0"
  ...
  Scenario: Resolve caches automatically
    ...
    Given the registry is unavailable
    When I run consumer command "rx ingest localhost:3098/cacheable:1.0.0"
    Then the command should succeed
  ...
  Scenario: Use latest tag when version omitted
    ...
    Then the command should succeed
    When I run consumer command "rx ingest localhost:3098/simple"
    Then the command should succeed
