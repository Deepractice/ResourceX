...
Feature: Collaboration between authors
  ...
  Scenario: Fork and modify someone's resource
    ...
    And I run "rx push original.text@1.0.0"

    # Another author pulls and creates derivative
    Given a fresh local environment
    When I run "rx pull original.text@1.0.0"
    ...
    And I run "rx push improved.text@1.0.0"
    Then the command should succeed

    # Both exist on registry
    When I run "rx search original"
    Then the output should contain "original.text@1.0.0"
    When I run "rx search improved"
    Then the output should contain "improved.text@1.0.0"
  ...
  Scenario: Team maintains shared resource with versions
    ...
    And I run "rx push team-tool.text@1.0.0"

    # Team member 2 adds improvement
    Given a fresh local environment
    When I run "rx pull team-tool.text@1.0.0"
    ...
    And I run "rx push team-tool.text@1.1.0"

    # Team member 3 can see all versions
    Given a fresh local environment
    When I run "rx search team-tool"
    Then the output should contain "team-tool.text@1.0.0"
    And the output should contain "team-tool.text@1.1.0"
  ...
  Scenario: Resolve resource directly from registry URL
    ...
    And I run "rx push direct-access.text@1.0.0"

    # Fresh user can resolve without explicit pull
    Given a fresh local environment
    When I run "rx resolve direct-access.text@1.0.0"
    Then the command should succeed
  ...
  # e.g., push to localhost:3000 -> locator becomes localhost:3000/resource.text@1.0.0
  # This enables:
  # - rx pull localhost:3000/resource.text@1.0.0 (no --registry needed)
  # - rx resolve localhost:3000/resource.text@1.0.0 (auto-fetches from registry)
  # ============================================
  ...
  Scenario: Push adds registry prefix to locator
    ...
    And I run "rx push prefixed.text@1.0.0"
    Then the command should succeed
    # The output should show the full registry-prefixed locator
    And the output should contain "localhost:3099/prefixed.text@1.0.0"

  @pending @design-issue
  Scenario: Pull by registry-prefixed locator without --registry flag
    Given a remote resource "localhost:3099/autoprefixed.text@1.0.0" on the registry with content "Auto content"
    And a fresh local environment
    # Should auto-detect registry from locator
    When I run "rx pull localhost:3099/autoprefixed.text@1.0.0"
    Then the command should succeed
    And the output should contain "Pulled"

  @pending @design-issue
  Scenario: Resolve auto-fetches from registry based on locator prefix
    Given a remote resource "localhost:3099/autofetch.text@1.0.0" on the registry with content "Fetched content"
    And a fresh local environment
    # Should auto-fetch from registry based on locator prefix
    When I run "rx resolve localhost:3099/autofetch.text@1.0.0"
    Then the command should succeed
